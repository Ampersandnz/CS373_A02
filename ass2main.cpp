...
bool W_DOWN, Z_DOWN, A_DOWN, S_DOWN, PLUS_DOWN, MINUS_DOWN, R_DOWN, C_DOWN;
...
void display(void)
{
	...
	drawStringAtRasterPosition(hitString, 50, 680);
	drawStringAtRasterPosition(missString, 50, 700);

	if (gameOver){
		char scoreString[40];
		sprintf_s(scoreString,"Congratulations, your score is: %d :-)",numEggsHit);
		drawStringAtRasterPosition(scoreString, 50, 640);
	}
...
{
	if ((key == 'w' || key == 'W') && (!W_DOWN)) { W_DOWN = true; time_w_down = clock(); }
	if ((key == 'z' || key == 'Z') && (!Z_DOWN)) { Z_DOWN = true; time_z_down = clock(); }
	if ((key == 'a' || key == 'A') && (!A_DOWN)) { A_DOWN = true; time_a_down = clock(); }
	if ((key == 's' || key == 'S') && (!S_DOWN)) { S_DOWN = true; time_s_down = clock(); }
	if (key == '=' || key == '+') {
		if (fov > 5) {
			fov--;
			glMatrixMode(GL_PROJECTION);
			glLoadIdentity();
			gluPerspective(fov, (GLfloat)windowWidth / (GLfloat)windowHeight, 1.0, 35.0);
			gluLookAt(0, 0, 25, 0, 0.75f, 0, 0, 1, 0);
			glutPostRedisplay();
		}
	}
	...
	if ((key == 'r') || (key == 'R')) {
		robot.incrementBasketShapeAngle();
		robot._initBasketSurface();
	}
	if ((key == 'c') || (key == 'C')) {
		if (robot.curveMode == HERMITE) { 
			robot.curveMode = PARAMETRIC; 
		} else { 
			robot.curveMode = HERMITE; 
		}
		robot._initBasketSurface();
	}
}
...
void handleKeyRelease(unsigned char key, int x, int y)
{
	...
	if (key == '=') PLUS_DOWN = false;
	if (key == '-') MINUS_DOWN = false;
}
...
int main(int argc, char** argv)
{
	...
	glutCreateWindow("COMPSCI 373 - Assignment - Easter Egg Game");
